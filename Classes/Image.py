...
class Image:
    ...
    def Filtrate(image:np.ndarray,kernel:int=5)->tuple[np.ndarray,float]:
        ...
        usedTreshold, img = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU) #Filtrate iamage to be eather black or white pixels only
    ...
    def Contrast(image:np.ndarray,kernelSize:float=0.02)->float:
        ...
        nbrElemts=int(np.round(sortedImage.size*kernelSize))
        #Arrays
        darkestPixels=sortedImage[:nbrElemts]
        brightestPixel=sortedImage[-nbrElemts:]
        return float(np.average(brightestPixel)-np.average(darkestPixels))
