...
class QrReader:
    ...
    def DecodeWithPyzbar(image:np.ndarray)->tuple[int,str,float]:
        ...
        text=decodeWithPyzbar(QrReader.__ValidateImage(image))
        if text:
    ...
    def DecodeWithQReader(image:np.ndarray)->tuple[int,str,float]:
        ...
        startTime=t.perf_counter()
        text=QReader().detect_and_decode(QrReader.__ValidateImage(image))
        if text:
    ...
    def DecodeWithCv2QrReader(image:np.ndarray)->tuple[int,str,float]:
        ...
        startTime=t.perf_counter()
        try:
            data, points,_= cv2.QRCodeDetector().detectAndDecode(QrReader.__ValidateImage(image))
    ...
    def DecodeWithPylibdmtx(image:np.ndarray)->tuple[int,str,float]:
        ...
        startTime=t.perf_counter()
        text=decodeWithPylibdmtx(QrReader.__ValidateImage(image))
        if text:
