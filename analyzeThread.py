import queue,numpy as np, threading as th,os,cv2, time as t
from Classes import Image, configReader as cr, QrReader as qr, Database
...
def __DecodePyzbarAndCv2(image:np.ndarray):
    ...
    stateCv2,textCv2,timeCv2=qr.QrReader.DecodeWithCv2QrReader(image)
    statePyzbar,textPyzbar,timePyzbar=qr.QrReader.DecodeWithPyzbar(image)
...
def __DecodePylib(image:np.ndarray):
    ...
    statePylib,textPylib,timePylib=qr.QrReader.DecodeWithPylibdmtx(image)
    with __lockDecodedPylib:
        __queryDecodePylib=f"{textPylib}"

def __DecodeQReader(image:np.ndarray):
    global __queryDecodedQReader
    stateQReader,textQReader,timeQReader=qr.QrReader.DecodeWithQReader(image)
